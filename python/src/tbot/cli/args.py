...
def parse_flags(argv: list[str]) -> GlobalFlags:
    """Pull out --json, --help/-h, --host=, --port=. Returns the rest as positional."""
    ...
    positional = [
        a for a in argv
        if a not in skip and not a.startswith("--host=") and not a.startswith("--port=")
    ]
    return GlobalFlags(json_mode=json_mode, help_mode=help_mode, host=host, port=port, positional=positional)
